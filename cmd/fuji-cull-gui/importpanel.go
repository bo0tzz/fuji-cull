...
func (u *ui) rightText(f *ttf.Font, s string, c sdl.Color, right, y int32) {
	...
	if err != nil {
		u.text(f, s, c, right, y, false)
		return
	}
	u.text(f, s, c, right-int32(w), y, false)
}
...
func (u *ui) drawImportPanel() {
	...
	if st.Running || st.Phase == "done" || st.Phase == "error" {
		...
		const labelW, numW = 62, 250
		barX := box.X + sc(24+labelW)
		barW := pw - sc(48+labelW+numW)
		...
		if st.FileTotal > 0 {
			...
			track := sdl.Rect{X: barX, Y: y + sc(6), W: barW, H: sc(3)}
			u.fillRect(track, colBG)
			fill := track
			fill.W = int32(float64(barW) * minf(float64(st.FileSent)/float64(st.FileTotal), 1))
